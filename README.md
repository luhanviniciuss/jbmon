...
- **Craft** (Bolsa, tecla B): Pokémon derrotados/capturados soltam Bolotas e Fragmentos (mais raros = mais Fragmentos). Receitas em `public/items.js`: Pokébola x3 = 3 Bolotas; Great = 4 Bolotas + 1 Fragmento; Ultra = 6 + 3 e Master = 30 + 12 estão **bloqueadas** ("Em breve", 🔒): Ultra Ball e Master Ball não saem mais do craft nem de drops e só virão em **eventos futuros** (o servidor recusa o craft; quem já tem continua podendo usar). O craft é bloqueado durante batalhas.
...
- **Grupo (tecla G)**: o líder convida jogadores online pelo nome (2 a 4 membros). Só grupos de 2+ conseguem iniciar a luta, e **sempre com o grupo inteiro** (2, 3 ou 4: nunca começa com só parte dos membros; o jogo avisa quem está faltando): todos os membros precisam estar perto do boss (~20 tiles) e um deles encostar nele.
- **Fase 1, a luta**: cada membro age na sua vez (25 s por turno) com o Pokémon ativo; o boss revida em quem atacou. O boss é sempre do **nível máximo do mundo** (Rota Lv.60, Gelo Lv.200, Vulcão Lv.500; avisa se a média do grupo for baixa) e a vida é multiplicada. Se todos forem eliminados, o boss vence e o grupo volta curado ao Centro.
...
- **Prêmios** (Ultra/Master Ball só saem em eventos): o **1º lugar sempre ganha 1 Master Ball** (mais 3 Ultra Balls, Bolotas e Fragmentos); 2º e 3º ganham Ultra Balls; 5+ acertos, Great Balls; participar dá Bolotas. Com menos de 2 jogadores não há prêmio de colocação.
- **Desempate**: se o 1º lugar termina empatado, rolam **perguntas extras só para os empatados** (morte súbita): quem erra ou não responde sai, se ao menos um acertou; os demais assistem. Repete até sobrar um (máx. 8 rodadas; depois vence o mais rápido).
...
- **Mundos** (cada um com 100x100 tiles, gerados por `generateMap(mundo)` em `public/map.js`, iguais no servidor e no cliente):
  ...
  - **Bioma de Gelo**: chão de neve, gelo liso, blocos de gelo, lago congelado e neve funda (encontros); neve caindo e tom frio. Selvagens de **Gelo/Água** (Smoochum, Sneasel, Jynx, Seel, Dewgong, Shellder, Cloyster, Lapras…), **Lv. 80 a 200**.
  - **Vulcão**: rocha escura, cinzas, rios de lava, rochas e chão quente com fissuras brilhando; brasas subindo e calor pulsando. Selvagens de **Fogo/Terra/Pedra** (Charmander, Vulpix, Growlithe, Ponyta, Magmar, Houndour, Geodude, Onix, Rhyhorn, Larvitar…), **Lv. 300 a 500**.
...
- **Selvagens sempre alcançáveis**: os Pokémon só nascem onde o jogador consegue chegar a pé a partir da chegada do mundo (busca em largura no mapa); rios de lava, lagos e árvores que isolam um pedaço do mapa não recebem selvagens.
- **Como funciona**: cada mundo é uma sala do Socket.io (`w:<mundo>`): só quem está nele vê seus jogadores e selvagens (`server/world.js`: mapa, spawn, respawn e vagar dos selvagens por mundo). O servidor valida a porta, troca o mundo e **salva o mundo do jogador** (coluna `User.world`): ao voltar ele reaparece onde estava. Cada mundo de combate (Rota, Gelo e Vulcão) tem o **seu boss lendário**, no nível máximo do mundo e só com lendários daquele mundo (Rota: Zapdos, Raikou, Mew, Celebi, Mewtwo, Latias, Latios, Jirachi, Deoxys; Gelo: Articuno, Suicune, Regice, Registeel, Lugia, Kyogre; Vulcão: Moltres, Entei, Regirock, Ho-Oh, Groudon, Rayquaza). A cada 3 h nasce um em cada mundo, sempre em terra que o jogador alcança; a raid acontece no mundo do boss, e o chip do HUD avisa de lendários em outros mundos. Perder uma batalha nos mundos extras leva ao Centro da Rota.
...
- **Fila de PvP** (Arena → aba **Fila PvP**): em vez de escolher adversário numa lista, você escolhe o modo (**Solo 1x1**, **Grupo** ou **Guerra de clãs**) e toca **"Entrar na fila"**. O servidor junta quem tem o **mesmo tamanho** (1x1, 2x2, 3x3, 4x4) e **rating parecido** (a tolerância começa em ±100 e cresce 40 por segundo de espera); na guerra de clãs os dois lados precisam ser clãs diferentes. Grupo: só o líder enfileira, e o grupo inteiro entra; guerra: líder/oficial. Enquanto espera aparece o chip **"🔎 Fila 0:12"** no topo (toque para abrir a Arena) e você pode andar à vontade; quem está em batalha/raid/interior é pulado até ficar livre. Sai da fila ao cancelar, desconectar (ou alguém do grupo sair) ou depois de 10 min. **Desafiar um jogador específico** continua possível: toque nele no mapa.
